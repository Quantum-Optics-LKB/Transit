...
for i in range(10):
    ...
    solver1 = temporal_bloch(T, I*(np.pi*waists_murad[i]**2), waists_murad[i], detun, L, N_grid=N_grid,
                                 N_v=N_v, N_real=N_real, N_proc=N_proc)
    pv = np.sqrt(2.0/np.pi)*((solver.m87/(Boltzmann*T))**(3.0/2.0))*Vs**2.0*np.exp(-solver.m87*Vs**2.0/(2.0*Boltzmann*T))
    dz = solver1.L/N_steps_z
    alpha = -np.log(0.6)/solver1.L
    for j in range(10):
        chihigh = np.sum(np.array(list(map(solver1.chi_analytical, Vs))).transpose(1, 2, 0)*dv*pv, axis=-1)
        chilow = np.sum(np.array(list(map(solver.chi_analytical, Vs))).transpose(1, 2, 0)*dv*pv, axis=-1)
        nhigh = np.sqrt(1+chihigh)
        nlow = np.sqrt(1+chilow)
        tab_analytical = np.real(nhigh-nlow)
        chihigh2 = np.sum(np.array(list(map(solver1.chi_analytical_2, Vs))).transpose(1, 2, 0)*dv*pv, axis=-1)
        chilow2 = np.sum(np.array(list(map(solver.chi_analytical_2, Vs))).transpose(1, 2, 0)*dv*pv, axis=-1)
        ...
        # tab = np.load(f"{path}/Dn_w{i}_z{j}_murad_Thu Feb  3 15:57:01 2022.npy") # gg
        tab = np.load(f"{path}/Dn_w{i}_z{j}_murad_Wed Feb  9 20:37:58 2022.npy")
        Dn += tab/N_steps_z
        Dn[Dn>0] = -1e-20
...
popt_lin, cov_lin = curve_fit(lin_fit, np.log(waists_murad), np.log(np.abs(Dn_c)))
...
popt1_lin, cov1_lin = curve_fit(lin_fit, np.log(waists_murad), np.log(n2_murad*Itilde))
fitted_exp = np.exp(lin_fit(np.log(waists_murad), popt1_lin[0], popt1_lin[1]))
popt2_lin, cov2_lin = curve_fit(lin_fit, np.log(waists_murad), np.log(np.abs(Dn_analytical_c2)))
popt3_lin, cov3_lin = curve_fit(lin_fit, np.log(waists_murad), np.log(np.abs(Dn_analytical_c)))
...
ax1.plot(waists_murad, fitted_comp,
         label=f"Lin fit comp : p = {'{:.2f}'.format(popt_lin[0])} "+"$\pm$ "+'{:.2f}'.format(perr_lin[0]),
         color="tab:blue") 
...
ax1.plot(waists_murad, fitted_exp,
         label=f"Lin fit exp : p = {'{:.2f}'.format(popt1_lin[0])}"+"$\pm$ "+'{:.2f}'.format(perr1_lin[0]),
         color="tab:orange")
...
fig1.suptitle("Non linear index $\Delta n$ against beam waist size", fontsize=20)
plt.show()