...
def open_tif(path, name):
    ...
    im = Image.open(path+'/'+'{}.tif'.format(name))
    return np.array(im)
...
def rad_avg(im, centre):
    ...
    Rx = np.abs(np.linspace(-centre_x, im.shape[1]-centre_x, 2048))
    Ry = np.abs(np.linspace(-centre_y, im.shape[0]-2048-centre_y, 2048))
